-- Update create_user_table_from_schema function to add unique constraint
...
BEGIN
  ...
  -- Check if columns include an 'id' field from the API
  FOR v_column IN SELECT * FROM jsonb_array_elements(p_columns)
  LOOP
    IF v_column->>'name' = 'id' THEN
      v_has_id_column := true;
    END IF;
  END LOOP;
  ...
  -- Add unique constraint if 'id' column exists
  IF v_has_id_column THEN
    v_sql := v_sql || ', CONSTRAINT unique_user_endpoint_record UNIQUE (user_id, source_endpoint_id, id)';
  END IF;
